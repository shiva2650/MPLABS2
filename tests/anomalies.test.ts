...
const mockProjects: Project[] = [
  {
    ...
    locationAddress: 'Ward 1, Hyderabad, Telangana',
    latitude: 17.3984,
    longitude: 78.5202,
    estimatedCost: 2100000,
    sanctionedAmount: 2100000,
    fundsUtilized: 1200000,
    implementingAgencyId: 'AGY-TEST-01',
    implementingAgencyName: 'Hyderabad Municipal Works Department',
    vendorName: 'Test Infrastructure Pvt Ltd',
    vendorPanMasked: 'ABCDE****F',
    recommendationDate: '2023-11-15',
    sanctionDate: '2023-12-15',
  ...
  },
  {
    ...
    locationAddress: 'Municipal Park Road, Ward 1, Hyderabad, Telangana',
    latitude: 17.3995,
...
export async function runAnomalyTests() {
  ...
  // With only a false-positive review, precision and F1 may correctly be zero.
  // Verify that retraining returns valid bounded metrics instead of treating zero
  // true positives as a successful detection result.
  test(
    'Model retraining returns valid empirical metrics after feedback',
    updatedMetadata.f1Score >= 0 && updatedMetadata.f1Score <= 1 &&
    updatedMetadata.rocAuc >= 0 && updatedMetadata.rocAuc <= 1
  );
