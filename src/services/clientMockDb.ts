...
export class ClientMockDbService {
  ...
  async login(userId: string, password: string): Promise<{ token: string; user: User; message: string }> {
    ...
    // Static hosting cannot provide a private authentication server. These hashed
    // demo credentials are intentionally limited to the GitHub Pages demo mode.
    ...
    const passwordValid = !!user && sha256Hex(rawPassword) === demoPasswordHashes[normalizedId];
